...
describe.only('format', () => {
    ...
    let range = Range.create(Position.create(0, 0), Position.create(beforeUArr.length - 1, beforeUArr[beforeUArr.length - 1].length));
    let before = format('\t', range, beforeU).split(/\r?\n/);
    it(`should not change lines num`, () => expect(before.length).to.be.equal(after.length));
    beforeUArr.forEach((l, i) => it(`should format "${l}" line properly`, () => {
